...
def gen_report_csv():
  ...
  df = pd.DataFrame(columns="features p r f acc".split(" "))
  ...
  for key, data in report.items():
    raw_cnt = len(data["raw"])
    for result in data["raw"].values():
      data["aggregate"]["p"] += result["p"]
      data["aggregate"]["r"] += result["r"]
      data["aggregate"]["f"] += result["f"]
      data["aggregate"]["acc"] += result["acc"]
      df.loc[i] = (key, result["p"], result["r"], result["f"], result["acc"])
      i += 1

  df.to_csv("memm_performance_result/report.csv")
